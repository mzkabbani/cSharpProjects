...
namespace Automation.Backend{
    public static class Rec_CapturePoints_TextConv {
        ...
        public static void DeletAllRespectiveCapturePointsForTextConversion(int captureEventId, SqlConnection conn, SqlTransaction transaction) {
            //pointRecId
            int value = 0;
            SqlCommand command = new SqlCommand(Rec_CapturePoints_TextConv_SQL.commandRemoveAllCapturePointsByRecIdTextConv, conn);
        ...
        public static int InsertCapturePointsForTextConversion(int RecommendationId, List<CustomTreeNode> customNodesList) {
            ...
            try {
                ...
                for (int i = 0; i < customNodesList.Count; i++) {
                    SqlCommand command = new SqlCommand(Rec_CapturePoints_TextConv_SQL.commandInsertCapturePointTextConv, conn);
                    ...
                    SqlCommand commandMaxId = new SqlCommand(Rec_CapturePoints_TextConv_SQL.commandMaxCapturePointIdTextConv, conn);
        ...
        public static int InsertCapturePointsAsTransactionForTextConversion(int RecommendationId, List<CustomTreeNode> customNodesList, SqlConnection conn, SqlTransaction transaction) {
            int returnCode = -1;
            List<int> capturePointsIds = new List<int>();
            for (int i = 0; i < customNodesList.Count; i++) {
                SqlCommand command = new SqlCommand(Rec_CapturePoints_TextConv_SQL.commandInsertCapturePointTextConv, conn);
        ...
        public static DataTable GetRespectiveCapturePointsForTextConversion(object advancedRecId, SqlConnection conn) {
            DataTable dataTable = new DataTable();//pointRecId
            SqlCommand command = new SqlCommand(Rec_CapturePoints_TextConv_SQL.commandGetAllRespectiveCapturePointsTextConv, conn);

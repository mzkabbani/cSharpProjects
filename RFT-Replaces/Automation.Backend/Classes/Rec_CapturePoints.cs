...
namespace Automation.Backend{
    public class Rec_CapturePoints {
        ...
        public static int InsertCapturePointsAsTransaction(int RecommendationId, List<CustomTreeNode> customNodesList, SqlConnection conn, SqlTransaction transaction) {
            int returnCode = -1;
            List<int> capturePointsIds = new List<int>();
            for (int i = 0; i < customNodesList.Count; i++) {
                SqlCommand command = new SqlCommand(Rec_CapturePoints_SQL.commandInsertCapturePoint, conn);
                ...
                SqlCommand commandMaxId = new SqlCommand(Rec_CapturePoints_SQL.commandMaxCapturePointId, conn);
                capturePointsIds.Add(Convert.ToInt32(commandMaxId.ExecuteScalar()));
            }
            return returnCode;
        }

        public static void DeletAllRespectiveCapturePoints(int captureEventId, SqlConnection conn, SqlTransaction transaction) {
            //pointRecId
            int value = 0;
            SqlCommand command = new SqlCommand(Rec_CapturePoints_SQL.commandRemoveAllCapturePointsByRecId, conn);
            command.Transaction = transaction;
        ...
        public static DataTable GetRespectiveCapturePoints(object advancedRecId, SqlConnection conn) {
            DataTable dataTable = new DataTable();//pointRecId
            SqlCommand command = new SqlCommand(Rec_CapturePoints_SQL.commandGetAllRespectiveCapturePoints, conn);
